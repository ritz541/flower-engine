...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        ...
        # Migration: Add scene if it doesn't exist
        try:
            cursor.execute(
        ...
        try:
            cursor.execute(
                "ALTER TABLE worlds ADD COLUMN system_prompt TEXT NOT NULL DEFAULT ''"
            )
        except sqlite3.OperationalError:
            pass  # Column already exists
        cursor.execute("""
...
class WorldManager:
    ...
    def add_world(self, world: World):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT OR REPLACE INTO worlds (id, name, lore, start_message, scene, system_prompt) VALUES (?, ?, ?, ?, ?, ?)",
                (
                    world.id,
                    world.name,
                    world.lore,
    ...
    def get_world(self, world_id: str) -> Optional[World]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT id, name, lore, start_message, scene, system_prompt FROM worlds WHERE id = ?",
                (world_id,),
            )
            row = cursor.fetchone()
            if row:
    ...
    def get_all_worlds(self) -> List[World]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT id, name, lore, start_message, scene, system_prompt FROM worlds"
            )
            return [
                World(
                    id=row[0],
...
class CharacterManager:
    ...
    def get_character(self, character_id: str) -> Optional[Character]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT id, name, persona FROM characters WHERE id = ?", (character_id,)
            )
            row = cursor.fetchone()
            if row:
    ...
    def list_recent(self, limit: int = 10) -> List[Session]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                ...
                "FROM sessions ORDER BY last_used_at DESC LIMIT ?",
                (limit,),
            )
            return [
                Session(
