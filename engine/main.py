...
async def startup():
    ...
    log.info(
        f"Engine state initialized: {len(state.available_worlds)} worlds, {len(state.available_characters)} characters."
    )
    ...
    try:
        ...
        log.error(f"Model fetch failed: {e}")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # RAG - reduce to keep context tight
                ...
                try:
                    ...
                    try:
                        ...
                        if cmd_msg.get("prompt") == "/cancel":
                            ...
                            await websocket.send_text(
                                build_ws_payload(
                                    "system_update", "✗ Stream cancelled by user."
                                )
                            )
                    except:
                        pass
