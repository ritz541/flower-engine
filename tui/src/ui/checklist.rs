...
pub fn draw(f: &mut Frame, app: &App, area: Rect) {
    let mut checklist = vec![
        ...
        Line::from(Span::styled(
            "  To begin your story, please prepare the stage:",
    ...
    let char_done = app.character_id != "Connecting..."
        && app.character_id != "Wanderer"
        && !app.character_id.is_empty();
    ...
    if !app.session_id.is_empty() {
        ...
        checklist.push(Line::from(Span::styled(
            "  ✦ The stage is set. Send a message to begin.",
            Style::default().fg(COLOR_AI).add_modifier(Modifier::ITALIC),
        )));
    }
    ...
    if !sys_msgs.is_empty() {
        ...
        for msg in sys_msgs.iter().rev() {
            let color = if msg.role == crate::app::Role::Error {
                crate::ui::COLOR_ERROR
            } else {
                Color::Indexed(239)
            };
            checklist.push(Line::from(vec![
    ...
    f.render_widget(
        Paragraph::new(checklist).block(
            Block::default().padding(Padding::vertical((area.height / 4).saturating_sub(2))),
        ),
        area,
    );
}
